...
while State.TouchFling and token == TouchFlingToken and not Library.Unloaded do
RunService.Heartbeat:Wait()
local root = GetRoot(LP)
if root then moveOffset = HiddenFlingPulse(root, moveOffset) end
end
end)
end
...
if not player or player == LP then Notify("Target Fling", "Select a target", "Warning") return false end
local root = GetRoot(LP)
...
local moveOffset = 0.1
while State.TargetFlingActive and os.clock() - started < State.TargetFlingDuration and not Library.Unloaded do
root = GetRoot(LP)
targetRoot = GetRoot(player)
if not root or not targetRoot or not IsCharacterAlive(player) then break end
...
root = GetRoot(LP)
if root then root.CFrame = oldCF root.AssemblyLinearVelocity = oldLinear root.AssemblyAngularVelocity = Vector3.zero end
State.TargetFlingActive = false
if State.Noclip and not State.FlingAllActive then task.defer(ApplyNoclip) end
return true
end
function FlingAll()
...
while State.TargetFlingActive do RunService.Heartbeat:Wait() end
FlingTarget(player)
task.wait(0.04)
end
end
root = GetRoot(LP)
if root and oldCF then root.CFrame = oldCF root.AssemblyLinearVelocity = Vector3.zero root.AssemblyAngularVelocity = Vector3.zero end
...
local root = GetRoot(LP)
if root then State.LastReturnCFrame = root.CFrame return true end
return false
end
...
local root = GetRoot(LP)
if not target or not targetRoot or not root then Notify("Target Teleport", "Selected target is unavailable", "Warning", 3, "Target") return false end
...
targetCF = CFrame.lookAt(position, targetRoot.Position, Vector3.yAxis)
else targetCF = targetRoot.CFrame end
...
local root = GetRoot(LP)
if not root or not State.LastReturnCFrame then Notify("Return Position", "No saved return position", "Warning") return false end
...
local names = {"None"}
for name in pairs(State.PositionBookmarks) do names[#names + 1] = name end
...
if BookmarkChoice and BookmarkChoice.SetValues then BookmarkChoice:SetValues(GetBookmarkNames(), true) State.SelectedBookmark = BookmarkChoice:Get() end
end
function SaveBookmark(name, cframe)
name = tostring(name or ""):gsub("^%s+", ""):gsub("%s+$", "")
if name == "" or name == "None" then State.BookmarkCounter += 1 name = "Bookmark " .. tostring(State.BookmarkCounter) end
...
local root = GetRoot(LP)
if not cframe or not root then Notify("Bookmark", "Bookmark is unavailable", "Warning") return false end
State.LastReturnCFrame = root.CFrame
root.CFrame = cframe
root.AssemblyLinearVelocity = Vector3.zero
root.AssemblyAngularVelocity = Vector3.zero
...
local presets = {Normal = {Walk = 16, Jump = 50, Fly = 50}, Fast = {Walk = 40, Jump = 90, Fly = 100}, Insane = {Walk = 120, Jump = 180, Fly = 220}}
local preset = presets[name]
if not preset then return end
State.WalkSpeedValue = preset.Walk
State.JumpPowerValue = preset.Jump
State.FlySpeed = preset.Fly
if Controls.WalkSpeedValue and Controls.WalkSpeedValue.Set then Controls.WalkSpeedValue:Set(preset.Walk, true) end
if Controls.JumpPowerValue and Controls.JumpPowerValue.Set then Controls.JumpPowerValue:Set(preset.Jump, true) end
if Controls.FlySpeedControl and Controls.FlySpeedControl.Set then Controls.FlySpeedControl:Set(preset.Fly, true) end
end
function ApplyESPPreset(name)
State.ESPPreset = name
local presets = {Minimal = {"DisplayName", "Role", "Distance", "Offscreen Arrows"}, Full = {"Highlight", "DisplayName", "Role", "Health Text", "Health Bar", "Distance", "Weapon", "2D Box", "Offscreen Arrows"}, Performance = {"Highlight", "DisplayName", "Role", "Distance"}, ["Role Only"] = {"Highlight", "Role", "Offscreen Arrows"}}
...
if Library.Root and Library.Root:IsA("ScreenGui") then Library.Root.Enabled = not State.ScreenshotMode end
if TargetTracerDrawing and State.ScreenshotMode then TargetTracerDrawing.Visible = false end
if State.ScreenshotMode then
CleanupGunESP()
CleanupCoins()
ClearHeatmap()
pcall(function() Library:SetMenuVisible(false) end)
end
end
...
TargetHUD.Visible = State.TargetLockHUD and not State.MinimalHUD and not State.ScreenshotMode and target ~= nil and root ~= nil
...
TargetHUDText.Text = string.format("%s\nRole: %s | HP: %d/%d\nDistance: %d | Speed: %.0f", GetStreamerDisplayName(target), tostring(GetRole(target)), math.floor(health + 0.5), math.floor(maxHealth + 0.5), math.floor(distance + 0.5), root.AssemblyLinearVelocity.Magnitude)
end
if State.ScreenshotMode then TargetArrowGui.Visible = false if TargetTracerDrawing then TargetTracerDrawing.Visible = false end return end
if State.TargetArrow and target and root and target ~= LP then SetEdgeArrow(TargetArrowGui, root.Position, State.ESPTargetColor, 0.36) else TargetArrowGui.Visible = false end
...
else TargetTracerDrawing.Visible = false end
else TargetTracerDrawing.Visible = false end
end
GunHUD.Visible = (State.DroppedGunDistanceHUD or State.MinimalHUD) and not State.ScreenshotMode
if State.MinimalHUD and not State.DroppedGunDistanceHUD then
if CachedDroppedGun then GunHUDText.Text = string.format("Gun: %d studs", math.floor(CachedDroppedGunDistance + 0.5)) else GunHUDText.Text = "Gun: not dropped" end
end
end)
