...
table.insert(GhostMotorMap, {Real = object, Ghost = table.remove(list, 1)})
end
end
end
end
...
if not ghostRoot or not ghostHumanoid then clone:Destroy() return false end
...
if realHumanoid then realHumanoid.PlatformStand = false realHumanoid.AutoRotate = true end
SetRealHidden(false)
Camera = Workspace.CurrentCamera or Camera
...
if not GhostModel or not GhostRoot or not GhostRoot.Parent then if not EnableGhost() then return end end
local humanoid = GetHumanoid(LP)
...
if IsKeyDown(Enum.KeyCode.LeftControl) or IsKeyDown(Enum.KeyCode.RightControl) or IsKeyDown(Enum.KeyCode.C) then vertical -= 1 end
local delta = Vector3.new(move.X, 0, move.Z) + Vector3.new(0, vertical, 0)
...
if flat.Magnitude > 0.001 then rotation = CFrame.lookAt(Vector3.zero, flat.Unit, Vector3.yAxis).Rotation end
GhostRoot.CFrame = CFrame.new(position) * rotation
for _, pair in ipairs(GhostMotorMap) do
if pair.Real and pair.Real.Parent and pair.Ghost and pair.Ghost.Parent then pair.Ghost.Transform = pair.Real.Transform end
end
end)
SpectateChoice
SpectateToggle
...
local values = {"None"}
for _, player in ipairs(Players:GetPlayers()) do if player ~= LP then table.insert(values, player.Name) end end
...
FreecamOld = {CameraType = Camera.CameraType, CameraSubject = Camera.CameraSubject, CFrame = Camera.CFrame, FOV = Camera.FieldOfView, MouseBehavior = UIS.MouseBehavior}
...
if input.UserInputType == Enum.UserInputType.MouseButton2 then FreecamLooking = true UIS.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition end
end)
Connect(UIS.InputEnded, function(input)
if input.UserInputType == Enum.UserInputType.MouseButton2 then FreecamLooking = false if State.Freecam then UIS.MouseBehavior = Enum.MouseBehavior.Default end end
end)
Connect(RunService.RenderStepped, function(dt)
...
FreecamPitch = math.clamp(FreecamPitch - math.rad(mouseDelta.Y * State.FreecamSensitivity), math.rad(-89), math.rad(89))
end
...
if IsKeyDown(Enum.KeyCode.LeftControl) or IsKeyDown(Enum.KeyCode.RightControl) or IsKeyDown(Enum.KeyCode.C) then move += Vector3.new(0,-1,0) end
if move.Magnitude > 1 then move = move.Unit end
local speed = State.FreecamSpeed
if IsKeyDown(Enum.KeyCode.LeftShift) or IsKeyDown(Enum.KeyCode.RightShift) then speed *= State.FreecamBoost end
local position = FreecamCF.Position + rotation:VectorToWorldSpace(move) * speed * dt
...
rotation = CFrame.lookAt(position, targetRoot.Position, Vector3.yAxis).Rotation
...
State.FreecamSpeed = math.clamp(State.FreecamSpeed + direction * State.FreecamWheelStep, 5, 300)
if Controls.FreecamSpeedControl and Controls.FreecamSpeedControl.Set then Controls.FreecamSpeedControl:Set(State.FreecamSpeed, true) end
end
end)
AntiFlingCollisionCache = setmetatable({}, {__mode = "k"})
LastCollisionRefresh = 0
function RestoreAntiFlingCollision()
for part, oldValue in pairs(AntiFlingCollisionCache) do if part and part.Parent then pcall(function() part.CanCollide = oldValue end) end end
...
if AntiFlingCollisionCache[part] == nil then AntiFlingCollisionCache[part] = part.CanCollide end
...
local intentional = State.TouchFling or State.TargetFlingActive or State.FlingAllActive or State.Fly or State.GhostMode
if intentional then RestoreAntiFlingCollision() return end
ApplyAntiFlingCollision()
if root.AssemblyAngularVelocity.Magnitude > State.AntiFlingAngularThreshold then root.AssemblyAngularVelocity = Vector3.zero end
local velocity = root.AssemblyLinearVelocity
local horizontal = Vector3.new(velocity.X, 0, velocity.Z)
if horizontal.Magnitude > State.AntiFlingLinearThreshold then
local allowed = State.WalkSpeed and math.min(State.WalkSpeedValue * 1.35, 90) or 45
local safe = Vector3.zero
if humanoid.MoveDirection.Magnitude > 0.01 then safe = humanoid.MoveDirection.Unit * allowed end
root.AssemblyLinearVelocity = Vector3.new(safe.X, velocity.Y, safe.Z)
end
end)
TouchFlingToken = 0
...
if root and root.Parent then root.Velocity = oldVelocity + Vector3.new(0, moveOffset, 0) end
return -moveOffset
end
